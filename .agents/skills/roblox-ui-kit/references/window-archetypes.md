...
|---|---|---|---|
| **Shop (Robux: passes + dev products)** | Portrait or narrow landscape, VERTICAL LIST | Sections with headers (Featured / Game Passes / Products); each item = full-width row: icon plate left, name + short description, green price button right (Robux/coin + amount); owned → button becomes blue disabled "OWNED"; currency packs may be a 2-3 col mini-grid inside their section | SectionHeader, ShopItemRow, PriceButton, FeaturedBanner |
...
| **Quests / Missions** | Portrait or landscape, VERTICAL LIST | Row per quest: name + description, progress bar with "3/10" text, reward icon + amount, CLAIM button (green when ready, gray/blue when not) | QuestRow, ProgressBar, ClaimButton state |
...
This is the reference answer for "сделай магазин с дев продуктами и пассами".
Portrait panel, vertical sectioned list. NOT a card grid.
