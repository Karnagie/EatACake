...
| `cake_variant_scenario.lua` | mode unused — selectable rainbow composition, terraces, material, wax/crust metadata and find payout |
| `round_cake_scenario.lua` | mode unused — leader selection through versioned TeleportData and destination round state |
...
SCENARIO_FILE=cake_variant_scenario.lua python tools/headless-sim/build_sim.py && luau tools/headless-sim/sim.luau
SCENARIO_FILE=round_cake_scenario.lua python tools/headless-sim/build_sim.py && luau tools/headless-sim/sim.luau
```
...
| `cake_variant_scenario.lua` | drives the REAL cake cycle + field services for classic/rainbow invariants and stepped-mask initialization |
| `round_cake_scenario.lua` | drives the REAL launch + round-validation modules for the cross-place cake-id protocol |
