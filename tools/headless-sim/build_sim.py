...
MODULE_SETS = {
    "treasure_scenario.lua": [
        ...
        # ⚠ BEFORE CakeConfig: CakeConfig requires it at load time (the layer
        # library + the flavour groups moved out 2026-08-07).
        ("Shared.config.CakeLayersConfig", "src/shared/config/CakeLayersConfig.lua"),
...
MODULE_SETS["pacing_scenario.lua"] = MODULE_SETS["treasure_scenario.lua"] + [
    ...
    # Section E measures where the clear time goes PER FLAVOUR ZONE, so it needs
    # the REAL `CakeCycleService.RollComposition` (a re-implementation would be
    # measuring the model, not the game) and therefore the data modules it takes
    # in Init. CakeConfigData pulls in every shared config, hence Body/Pet below.
    ...
    ("__CakeCycleService", "src/server/game/services/CakeCycleService.lua"),
]
...
MODULE_SETS["cake_variant_scenario.lua"] = MODULE_SETS["layereater_scenario.lua"] + [
    ("Shared.config.MatchConfig", "src/shared/config/MatchConfig.lua"),
    ("Shared.config.AnalyticsConfig", "src/shared/config/AnalyticsConfig.lua"),
    ("__CakeCycleService", "src/server/game/services/CakeCycleService.lua"),
    ("__CakeCycleSubs", "src/server/game/subscriptions/CakeCycleSubs.lua"),
    ("__CakeSimulationSubs", "src/server/game/subscriptions/CakeSimulationSubs.lua"),
]
MODULE_SETS["round_cake_scenario.lua"] = [
    ("Shared.config.CakeLayersConfig", "src/shared/config/CakeLayersConfig.lua"),
    ("Shared.config.CakeConfig", "src/shared/config/CakeConfig.lua"),
    ("Shared.config.CakeSelectConfig", "src/shared/config/CakeSelectConfig.lua"),
    ("Shared.config.MatchConfig", "src/shared/config/MatchConfig.lua"),
    ("Shared.config.PlaceConfig", "src/shared/config/PlaceConfig.lua"),
    ("__RoundStateData", "src/server/game/data/RoundStateData.lua"),
    ("__GameRoundService", "src/server/game/services/GameRoundService.lua"),
    ("__LobbyQueueLaunch", "src/server/lobby/subscriptions/LobbyQueue/Launch.lua"),
]
...
if SCENARIO_FILE == "cake_variant_scenario.lua":
    # The real cycle/simulation subscriptions only need outbound RemoteEvents in
    # this scenario. Record those fires so tests can inspect transition payloads.
    out.append(
        r'''
__REMOTE_CALLS = {}
__REGISTRY["Shared.Net"] = {
	Update = function(name)
		return {
			FireAllClients = function(_, ...)
				table.insert(__REMOTE_CALLS, { name = name, kind = "all", args = { ... } })
			end,
			FireClient = function(_, player, ...)
				table.insert(__REMOTE_CALLS, { name = name, kind = "client", player = player, args = { ... } })
			end,
		}
	end,
}
